...
func TestSelfHostedModeLocalhost(t *testing.T) {
	config := AxonFlowConfig{
		AgentURL: "http://localhost:8080",
		// No ClientID, ClientSecret, or LicenseKey provided
	}

	client := NewClient(config)

	if client == nil {
		t.Fatal("Expected client to be created for localhost without auth, got nil")
	}

	// Localhost should default to sandbox mode
	if client.config.Mode != "sandbox" {
		t.Errorf("Expected localhost to default to 'sandbox' mode, got '%s'", client.config.Mode)
	}
}

func TestSelfHostedMode127(t *testing.T) {
	config := AxonFlowConfig{
	...
	if client == nil {
		t.Fatal("Expected client to be created for 127.0.0.1 without auth, got nil")
	}

	// 127.0.0.1 should also default to sandbox mode
	if client.config.Mode != "sandbox" {
		t.Errorf("Expected 127.0.0.1 to default to 'sandbox' mode, got '%s'", client.config.Mode)
	}
}

func TestSelfHostedModeExplicitProduction(t *testing.T) {
	config := AxonFlowConfig{
		AgentURL: "http://localhost:8080",
	...
	// Should respect explicit mode setting
	if client.config.Mode != "production" {
		t.Errorf("Expected explicitly set 'production' mode, got '%s'", client.config.Mode)
	}
}

func TestSelfHostedModeWithOptionalLicenseKey(t *testing.T) {
	config := AxonFlowConfig{
		AgentURL:   "http://localhost:8080",
		LicenseKey: "test-license-key", // Optional - can still provide for localhost
	}
	...
	if client.config.LicenseKey != "test-license-key" {
		t.Errorf("Expected LicenseKey to be preserved, got '%s'", client.config.LicenseKey)
	}
}

func TestSelfHostedModeVariousURLs(t *testing.T) {
	testCases := []string{
		"http://localhost:8080",
	...
	for _, url := range testCases {
		t.Run(url, func(t *testing.T) {
			...
			if client == nil {
				t.Fatalf("Expected client to be created for URL %s, got nil", url)
			}

			// All localhost URLs should default to sandbox
			if client.config.Mode != "sandbox" {
				t.Errorf("Expected sandbox mode for URL %s, got '%s'", url, client.config.Mode)
			}
		})
	}
}
...
func TestNonLocalhostRequiresAuth(t *testing.T) {
	config := AxonFlowConfig{
		...
		// Intentionally not providing auth to verify behavior
	}

	client := NewClient(config)

	// Client should be created but will fail when making requests without auth
	if client == nil {
		t.Fatal("Expected client to be created, got nil")
	}

	// Non-localhost should default to production
	if client.config.Mode != "production" {
		t.Errorf("Expected production mode for non-localhost, got '%s'", client.config.Mode)
	}
}
